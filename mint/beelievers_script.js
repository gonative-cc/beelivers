...
async function addPartners(partners) {
	...
	txb.moveCall({
		...
		arguments: [txb.object(ADMIN_CAP), txb.object(COLLECTION_ID), txb.pure(partners)],
	});
...
async function setBadgeDisplayable(badgeName, displayable) {
	...
	try {
		...
		console.log(
			`Successfully set badge ${badgeName} displayable to ${displayable}:`,
			result.digest,
		);
		return result;
	} catch (error) {
...
async function executeNativePremint() {
	...
	try {
		...
		console.log(
			`🎉 Successfully completed all premint batches! Total: ${completedPremint} NFTs`,
		);
		return true;
	} catch (error) {
...
async function startMinting(startTime) {
	...
	txb.moveCall({
		...
		arguments: [txb.object(ADMIN_CAP), txb.object(COLLECTION_ID), txb.pure(startTime)],
	});
...
async function pauseMinting() {
	...
	txb.moveCall({
		...
		arguments: [txb.object(ADMIN_CAP), txb.object(COLLECTION_ID)],
	});
		...
		for (let i = 1; i <= 6021; i += BATCH_SIZE) {
			...
			if (tokenIds.length > 0) {
				...
				try {
					...
					console.log(
						`Successfully processed ${tokenIds.length} badges in current batch`,
					);
	...
	try {
		switch (operation) {
			...
			case "add-partners":
				if (!filePath) {
					console.error(
						"Please provide a file path: node beelievers_script.js add-partners <file_path>",
					);
					break;
				}
			...
			case "set-auction-contract":
				if (!filePath) {
					console.error(
						"Please provide auction contract address: node beelievers_script.js set-auction-contract <address>",
					);
					break;
				}
			...
			case "add-badge":
				if (!filePath) {
					console.error(
						"Please provide token ID and badge: node beelievers_script.js add-badge <token_id,badge>",
					);
					break;
				}
			...
			case "add-badges":
				if (!filePath) {
					console.error(
						"Please provide token ID and badges: node beelievers_script.js add-badges <token_id,badge1,badge2,badge3>",
					);
					break;
				}
			...
			case "set-badge-display":
				if (!filePath) {
					console.error(
						"Please provide badge name and displayable: node beelievers_script.js set-badge-display <badge_name> <true/false>",
					);
					break;
				}
			...
			case "start-minting":
				if (!filePath) {
					console.error(
						"Please provide start time: node beelievers_script.js start-minting <timestamp>",
					);
					break;
				}
