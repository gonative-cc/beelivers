...
const EWrongAuctionContract: u64 = 11;
...
fun init(witness: MINT, ctx: &mut TxContext) {
    ...
    display::add(
        ...
        string::utf8(
            b"BTCFi Beelievers is more than an NFT- it's a movement to make Bitcoin work in DeFi without bridges, wrappers, or custodians. The Beeliever NFT is your badge of conviction, fueling Native's nBTC and BYield on Sui.",
        ),
    );
    display::add(
        &mut nft_display,
    ...
    let (transfer_policy, transfer_policy_cap) = transfer_policy::new<BeelieverNFT>(
        &publisher,
        ctx,
    );
...
fun create_nft(
    ...
    let attributes = if (
        table::contains<u64, VecMap<String, String>>(&collection.nft_metadata, token_id)
    ) {
...
fun select_random_mythic(
    ...
    let random_index = (
        random::generate_u64_in_range(&mut generator, 0, (available_length as u64)) as u64,
    );
...
fun roll_for_mythic(collection: &BeelieversCollection, random: &Random, ctx: &mut TxContext): bool {
    let mut generator = random::new_generator(random, ctx);
...
public entry fun set_bulk_nft_attributes(
    ...
    assert!(
        total_nfts == vector::length(&keys) && total_nfts == vector::length(&values),
...
public entry fun mint(
    ...
    let (is_eligible, is_mythic) = determine_mint_eligibility(
        collection,
        sender,
        random,
        auction,
...
fun determine_mint_eligibility(
    ...
    auction: &Auction,
    ctx: &mut TxContext,
): (bool, bool) { if (is_partner(collection, sender)) {
